"""Run every demo scenario and report what the InefficiencyDetector finds.

Offline and deterministic: no network, no API keys, no external services.

Usage::

    python -m examples.detect_inefficiencies
"""
...
def main() -> None:
    ...
    for name, scenario in SCENARIOS.items():
        ...
        print(f"{name}: {len(issues)} inefficiency issue(s)")
        for issue in issues:
            print(

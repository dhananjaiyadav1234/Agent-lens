"""Run every demo scenario and report what the LoopDetector finds.

Offline and deterministic: no network, no API keys, no external services.

Usage::

    python -m examples.detect_loops
"""
...
def main() -> None:
    ...
    for name, scenario in SCENARIOS.items():
        ...
        print(f"{name}: {len(issues)} loop issue(s)")
        for issue in issues:
            print(
                f"  {issue.issue_type.value} / {issue.severity.value} / "
                f"cycle_length={issue.metadata['cycle_length']} "
                f"repetitions={issue.metadata['repetitions']} "
                f"related_events={len(issue.related_event_ids)}"
            )
            print(f"  pattern: {issue.metadata['pattern']}")
